...
class TorchSpectralModel2D(AbstractCfdModel):
    def __init__(
        ...
        *,
        ...
        self.nx = nx
        self.ny = ny
        self.coeff_linear_drag = coeff_linear_drag
        self.coef_diffusion = coef_diffusion
        self.order_diffusion = order_diffusion
        self.device = device

        logger.info(
            f"TorchSpectralModel2D for periodic domain: {nx=}, {ny=}, {coeff_linear_drag=}, {coef_diffusion=}, {order_diffusion=}, {device=}, {dtype=}"
        )
        ...
        self.__time_integrator = runge_kutta_2nd_order
        self.__diffusion_operator = coeff_linear_drag + coef_diffusion * torch.pow(
            self.__fft.k2, order_diffusion
        )
    ...
    def initialize(
        ...
        t0: float,
        omega0: torch.Tensor,
        forcing: typing.Optional[torch.Tensor] = None,
    ):
        assert isinstance(t0, float)
        assert omega0.ndim == 3  # batch, x, y dims
        assert omega0.shape[-2] == self.nx
        ...
        if forcing is not None:
            ...
            assert forcing.shape[0] == 1 or forcing.shape[0] == omega0.shape[0]
            assert forcing.shape[-2] == self.nx
            assert forcing.shape[-1] == self.ny
    ...
    def time_integrate(self, dt: float, nt: int, hide_progress_bar: bool = False):
        for _ in tqdm(range(nt), disable=hide_progress_bar):
            ...
            has_nan = torch.any(torch.isnan(self.spec_omega)).item()
            if has_nan:
